...
class PDFToolbox:
    def __init__(self, root):
        self.root = root
        self.root.title("My PDF Auto")
        self.root.geometry("400x400")
        self.root.configure(bg="#f0f0f0")

        # --- CONFIGURATION ---
        # To add more tools in the future, just add them to this list:
        # Format: {"name": "Button Label", "file": "filename.py", "desc": "Description"}
        self.tools = [
            {
                "name": "PDF Merger", 
                "file": "pdf_merge.py", 
        ...
        # Loop through the list and create buttons dynamically
        for tool in self.tools:
            self.create_tool_button(container, tool)

        # --- FOOTER ---
        footer = tk.Label(root, text="Ready", bd=1, relief=tk.SUNKEN, anchor=tk.W)
        footer.pack(side=tk.BOTTOM, fill=tk.X)
        self.status_label = footer

    def create_tool_button(self, parent, tool_info):
        """Creates a nice card-like button for each tool"""
        
        frame = tk.Frame(parent, bg="white", bd=2, relief=tk.RAISED)
        frame.pack(fill=tk.X, pady=5)

        # Inner logic to make the whole frame clickable
        def on_click(e=None):
